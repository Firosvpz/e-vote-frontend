...
const VotersModal = ({ isOpen, onClose, voters }) => {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center  transition-opacity duration-300">
            <div className="relative  bg-gradient-to-br from-slate-900 via-purple-900 to-slate-600 rounded-lg shadow-2xl w-full max-w-xl p-6 max-h-[80vh] overflow-y-auto transform scale-95 data-[state=open]:scale-100 transition-transform duration-300">
                <div className="text-center mb-6 border-b pb-4">
                    <h2 className="text-2xl font-bold text-white">Eligible Voters</h2>
                    <p className="text-gray-400 mt-1">List of all voters eligible for this election.</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {voters && voters.length > 0 ? (
                        voters.map((voter, index) => (
                            <div key={voter._id || index} className="border border-gray-200 rounded-md p-4  shadow-sm">
                                <p className="font-semibold text-white">{voter.name}</p>
                                <p className="text-sm text-gray-400">Department: {voter.department}</p>
                            </div>
                        ))
                    ) : (
                        <p className="col-span-full text-center text-gray-600">No eligible voters found.</p>
                    )}
                </div>

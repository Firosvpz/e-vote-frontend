...
import { getResults } from "../../api/user/userApi"
  ...
  useEffect(() => {
    const fetchElectionResults = async () => {
      ...
      try {
            ...
            .map((resultItem) => {
              ...
              const candidates = resultItem.voteBreakdown.map((breakdown, index) => {
                ...
                return {
                  ...
                  major: breakdown.candidate.department, // Mapping department to major
                  year: breakdown.candidate.academicLevel, // Mapping academicLevel to year
              ...
              return {
                ...
                totalVotes: resultItem.totalVotes, // From outer result object
                participationRate: resultItem.turnoutPercentage, // From outer result object
        ...
        console.error(err?.message || "An error occurred while fetching results", err)
        ...
        {selectedElectionId === null ? (
          ...
          currentElection && (
            <>
              ...
              {/* Additional Analytics (only if data exists in backend response) */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                {currentElection.timeline && currentElection.timeline.length > 0 && (
                ...
                {currentElection.demographics && currentElection.demographics.length > 0 && (
                  <div className="bg-white/10 backdrop-blur-md rounded-3xl border border-white/20 p-8">
                    <h3 className="text-xl font-bold text-white mb-6">Voter Demographics</h3>
                    <div className="space-y-4">
                      {currentElection.demographics.map((demo, index) => {
                        const colorClasses = getColorClasses(demo.color) // Assuming demo.color exists or defaults
                        return (
